    ...
    try {
        if(walletAddress === null || walletAddress === '' || walletAddress === undefined) {
            if(parseInt(chainId) == 80001) {
                const simpleRpcProvider = new ethers.providers.StaticJsonRpcProvider('https://rpc-mumbai.maticvigil.com/')
            ...
            if(parseInt(chainId) == 137) {
                const simpleRpcProvider = new ethers.providers.StaticJsonRpcProvider('https://polygon-rpc.com/')
